...
interface ImageUploadSlotProps {
  label: string;
  value?: ReviewFormImage;
  onChange: (value: ReviewFormImage | undefined) => void;
  maxSizeMB?: number;
  readOnly?: boolean;
}
...
function MultiImageListItem({
  ...
  const { t } = useTranslation();
  const [zoomImage, setZoomImage] = useState("");
  const previewSrc = useMemo(() => getReviewFormImagePreviewSrc(img), [img]);
...
function MultiImageUploadSlot({
  ...
  const processFile = useCallback(
    async (file: File): Promise<ReviewFormImage | null> => {
...
export interface ReviewImagesUploadProps {
  ...
  onArtStyleImgChange: (value: ReviewFormImage[] | undefined) => void;
  readOnly?: boolean;
  maxSizeMB?: number;
}
...
interface VideoUploadSlotProps {
  ...
  onChange: (value: ReviewFormVideo[] | undefined) => void;
  maxSizeMB?: number;
  readOnly?: boolean;
}
...
function MultiVideoUploadSlot({
  ...
  const processFile = useCallback(
    async (file: File): Promise<ReviewFormVideo | null> => {
      if (readOnly) return null;
...
export interface ReviewVideoUploadProps {
  videoRef?: ReviewFormVideo[];
  onVideoRefChange: (value: ReviewFormVideo[] | undefined) => void;
  readOnly?: boolean;
  maxSizeMB?: number;
}

export function ReviewVideoUpload({
  videoRef,
  onVideoRefChange,
  readOnly = false,
  maxSizeMB = 100,
}: ReviewVideoUploadProps) {
  const { t } = useTranslation();
