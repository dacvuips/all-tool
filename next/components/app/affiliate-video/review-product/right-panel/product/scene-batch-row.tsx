...
export const SceneBatchRow = React.memo(function SceneBatchRow({
  ...
  const { scriptData, reviewFormConfig } = useReviewContext();
  ...
  const [selectedReviewImageSlots, setSelectedReviewImageSlots] = useState<
    (ReviewFormImage | undefined)[]
  >(scene.reviewImageSlots || []);
  ...
  useEffect(() => {
    setSelectedReviewImageSlots(scene.reviewImageSlots || []);
  }, [scene.id, scene.reviewImageSlots]);

  const handleReviewImageSlotsChange = useCallback(
    (slots: (ReviewFormImage | undefined)[]) => {
      const urls = reviewImageSlotsToUrls(slots);
      setSelectedReviewImageSlots(slots);
      setSelectedProductImages(urls);
      selectedProductImagesDB.set(scene.id, urls);
      onUpdateSelectedProductImages?.(scene.id, urls);
      onUpdateReviewImageSlots?.(scene.id, slots, urls);
    },
    [scene.id, selectedProductImagesDB, onUpdateSelectedProductImages, onUpdateReviewImageSlots]
  );
