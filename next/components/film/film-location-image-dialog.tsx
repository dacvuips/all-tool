/**
 * Dialog tạo ảnh Location — prompt instruction 4 views continuity sheet.
 */
...
export type FilmLocationImageGenerateInput = {
  item: FilmSceneImageRecord;
  prompt: string;
  /** Ảnh tham chiếu (vật phẩm set dressing) */
...
type Props = {
  isOpen: boolean;
  item: FilmSceneImageRecord | null;
  /** Aspect project — đưa vào template prompt */
  aspectRatio?: "16:9" | "9:16";
  /** Prompt mẫu Setting dự án */
  promptTemplate?: string | null;
  onClose: () => void;
  onGenerate: (input: FilmLocationImageGenerateInput) => Promise<void>;
};

export default function FilmLocationImageDialog({
  isOpen,
  item,
  aspectRatio = "16:9",
  promptTemplate,
  ...
  useEffect(() => {
    ...
    const next =
      ...
      buildFilmLocationImagePrompt(item, aspectRatio, promptTemplate);
    setPrompt(next);
    setSubmitting(false);
  }, [isOpen, item, aspectRatio, promptTemplate]);

  if (!item) return null;

  const alreadyCreated =
    item.status === "created" || !!item.imageUrl || (item.imageUrls?.length || 0) > 0;
  const title = alreadyCreated ? t("Tạo lại ảnh Location") : t("Tạo ảnh Location");
  const timeOfDay = item.timeOfDay?.trim() || item.context?.trim() || "—";
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      await onGenerate({ item, prompt: finalPrompt });
      onClose();
    } catch {
      // parent toast
    } finally {
          ...
          <div className="flex items-start justify-between gap-3 mb-4">
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-500 m-0 mt-1 truncate">
                ...
                <span className="text-gray-400"> · {timeOfDay}</span>
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
              <div className="rounded-xl border border-gray-100 bg-gray-50 px-3 py-2">
                ...
                <div className="text-gray-700 line-clamp-2">{timeOfDay}</div>
              </div>
              <div className="rounded-xl border border-gray-100 bg-gray-50 px-3 py-2">
                <div className="text-10 font-semibold text-gray-400 uppercase mb-0.5">
                ...
                <div className="text-gray-700 line-clamp-3">
                  {item.description?.trim() || t("Chưa có mô tả")}
                </div>
              </div>
            </div>
