...
// ── Main Component ──────────────────────────────────────────────────────
export const AppPromptRank = () => {
  const { t } = useTranslation();
  const { getFlowAppRank, getAiStudioAppRank } = useAffiliateVideoApi();
  ...
  const loadRank = useCallback(
    async (pageNum: number, search?: string) => {
      setIsLoading(true);
      try {
        const [flowResult, aiStudioResult] = await Promise.all([
          getFlowAppRank(pageNum, RANK_PER_PAGE, search || undefined),
          getAiStudioAppRank(pageNum, RANK_PER_PAGE, search || undefined),
        ]);
        const merged = [...flowResult.data, ...aiStudioResult.data].sort(
          (a, b) => (b.monthlyCount || 0) - (a.monthlyCount || 0)
        );
        setItems(merged);
        setTotal(flowResult.total + aiStudioResult.total);
      } catch {
        setItems([]);
        setTotal(0);
      } finally {
    ...
    [getFlowAppRank, getAiStudioAppRank]
  );
      ...
      <div className="overflow-y-auto flex-1 p-2 space-y-3 v-scrollbar xs:p-3">
        ...
        <div className="flex justify-between items-center">
          <div className="flex gap-2 items-center">
            ...
            <h3 className="m-0 text-sm font-bold text-gray-800">{t("Bảng xếp hạng App")}</h3>
          </div>
          <button
