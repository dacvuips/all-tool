...
interface UseSceneMediaParams {
  ...
  selectedReviewVideoSlots?: (ReviewFormVideo | undefined)[];
...
export function useReviewSceneMedia({
  scene,
  nextSceneId,
  thumbnailOriginImage,
  selectedProductImages,
  selectedReviewImageSlots,
  selectedReviewVideoSlots,
  noText,
  ...
  const handleGenerateVideoToVideo = async () => {
    ...
    const videoSlotsForApi =
      countFilledVideoSlots(selectedReviewVideoSlots) >=
      countFilledVideoSlots(scene.reviewVideoSlots)
        ? selectedReviewVideoSlots
        : scene.reviewVideoSlots ?? selectedReviewVideoSlots;

    const videoPayload = await resolveReviewReferenceVideoForApi({
    ...
    try {
      ...
      const slotsForVideo =
        countFilledSlots(selectedReviewImageSlots) >= countFilledSlots(scene.reviewImageSlots)
          ? selectedReviewImageSlots
          : scene.reviewImageSlots ?? selectedReviewImageSlots;

      imagesArray = await resolveReviewReferenceImagesForApi({
