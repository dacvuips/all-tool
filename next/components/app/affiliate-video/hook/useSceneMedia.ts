...
export function useSceneMedia({ scene, nextSceneId }: UseSceneMediaParams): UseSceneMediaReturn {
  ...
  const isBatchGeneratingExtendVideo =
    batchGeneratingVideoSceneIds?.has(scene.id + "::stitch") ?? false;
  ...
  }, [nextSceneId, getGeneratedImage, isBatchGenerating]);
  ...
  }, [scene.id, getGeneratedVideo, isBatchGeneratingExtendVideo]);
  ...
  const handleGenerateImage = async () => {
    ...
    const currentImageGenerating = batchGeneratingSceneIds?.size ?? 0;
  ...
  const handleGenerateVideo = async (isStitch?: boolean) => {
    ...
    // ── Check concurrency limit ──
    const currentVideoGenerating = batchGeneratingVideoSceneIds?.size ?? 0;
