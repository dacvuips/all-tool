...
interface VideoToVideoListPanelProps {
  scenes: CopyVideoScene[];
  characters: CharacterItem[];
  sceneListKey?: TabSceneListKey;
}

export function VideoToVideoListPanel({
  scenes,
  characters,
  sceneListKey = "videoToVideoScenes",
}: VideoToVideoListPanelProps) {
  const {
    scriptData,
    updateScriptData,
    selectedHistoryId,
  ...
  const handlePersistScenes = async (updatedScenes: any[]) => {
    try {
      ...
      console.error("[elements/VideoToVideoListPanel] Failed to persist to IndexedDB:", err);
    }
  };
  ...
  const handleSyncScenes = async (updatedScenes: any[]) => {
    try {
      ...
      console.error("[elements/VideoToVideoListPanel] Failed to sync:", err);
    }
  };
