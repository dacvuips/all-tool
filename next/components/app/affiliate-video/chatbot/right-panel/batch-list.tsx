/**
 * batch-list.tsx (chatbot)
 * Thin wrapper around SharedBatchListPanel for the "chatbot" module.
...
import { BatchActionBar } from "../../trending/right-panel/batch-action-bar";
...
export function BatchListPanel({ scenes, characters }: BatchListPanelProps) {
  ...
  const { introElement, openIntro } = useAffiliateBatchListIntro({
    sidebarIntroKey: IntroGuideKey.CHATBOT_SIDEBAR,
  ...
  const handlePersistScenes = async (updatedScenes: any[]) => {
    try {
      ...
      console.error("[chatbot/BatchListPanel] Failed to persist to IndexedDB:", err);
    }
  };
  ...
  const handleSyncScenes = async (updatedScenes: any[]) => {
    try {
      ...
      console.error("[chatbot/BatchListPanel] Failed to sync:", err);
    }
  };
  ...
  const handleBuildInsertedScene = async (
    ...
    try {
      ...
      console.error("[chatbot/handleInsert] API error:", err);
      return {
        id: crypto.randomUUID(),
        sceneNumber,
