...
interface ApiMediaResponseData {
  url: string;
  apiKey: string;
}

interface ApiMediaAPISetting {
  link: ApiMediaResponseData[];
}
...
export default [
  {
    ...
    action: async (req: Request, res: Response) => {
      ...
      const token = await apiMediaTokenService.findOne({ key: apiKey, active: true });
      if (!token) {
      ...
      let settingValue: ApiMediaAPISetting | undefined;
      try {
        settingValue =
          typeof setting?.value === "string"
            ? JSON.parse(setting.value)
            : (setting?.value as ApiMediaAPISetting | undefined);
      } catch {
      ...
      for (const selectedLink of links) {
        ...
        try {
          ...
          lastError = err;
          console.warn(
            `[api-media] Link ${selectedLink?.url} thất bại: ${err.message}. Thử link tiếp theo...`
          );
          continue;
        }
      }
