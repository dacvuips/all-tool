...
import { ArtStyleFields } from "./components/art-style-fields";
...
export function ArtStylePage() {
  ...
  return (
    <Card>
      <DataTable<ArtStyle> crudService={ArtStyleService} order={{ createdAt: -1 }}>
        <DataTable.Header>
          <DataTable.Title />
          <DataTable.Buttons>
            ...
            <>
              <DataTable.Table className="mt-4">
                ...
                <DataTable.Column
                  center
                  label={t("Lượt dùng")}
                  render={(item: ArtStyle) => <DataTable.CellNumber value={item.count} />}
                />
                <DataTable.Column
                  label={t("Customer ID")}
                  render={(item: ArtStyle) => <DataTable.CellText value={item.customerId} />}
                />
                <DataTable.Column
                  center
                  label={t("Công khai")}
                    ...
                    <DataTable.CellText
                      ...
                      value={
                        <Switch
                          ...
                          onChange={async () => {
                            try {
                              const res = await ArtStyleService.update({
                                id: item.id,
