...
const btn = (extra: React.CSSProperties = {}): React.CSSProperties => ({
  border: "none", borderRadius: CSS.radiusSm, cursor: "pointer",
  fontWeight: 600, fontSize: 13, transition: "all 0.15s", ...extra,
});
...
export default function VirtualTryOn() {
  ...
  /* ─── Single mode ─── */
  const [singleSlots, setSingleSlots] = useState<Record<ItemType, ItemSlot>>(makeDefaultSingleSlots);
  ...
  const updateBatchSlot = (outfitId: string, type: ItemType, patch: Partial<ItemSlot>) =>
    setBatches((prev) =>
      prev.map((o) =>
        o.id !== outfitId ? o : {
          ...o,
          slots: { ...o.slots, [type]: { type, image: null, prompt: "", ...(o.slots[type] ?? {}), ...patch } },
        }
      )
    );
  ...
  const generateBatch = async () => {
    ...
    await Promise.all(
      batches.map(async (outfit) => {
        ...
        if (!items.length) {
          setBatches((prev) => prev.map((o) => o.id === outfit.id ? { ...o, isGenerating: false, error: "Chưa chọn trang phục" } : o));
          return;
        }
        try {
          ...
          setBatches((prev) => prev.map((o) => o.id === outfit.id ? { ...o, result: image, isGenerating: false } : o));
          if (image) setHistory((h) => [{ id: uid(), ts: Date.now(), image }, ...h]);
        } catch (e: any) {
          setBatches((prev) => prev.map((o) => o.id === outfit.id ? { ...o, isGenerating: false, error: e.message } : o));
        }
      })
    );
  };
    ...
    <div style={{ minHeight: "100vh", background: CSS.gradBg, fontFamily: "'Inter', -apple-system, sans-serif", color: CSS.textPrimary }}>
      ...
      <div style={{ ...card({ borderRadius: 0, borderLeft: "none", borderRight: "none", borderTop: "none", padding: "12px 20px", display: "flex", alignItems: "center", gap: 16, position: "sticky", top: 0, zIndex: 100 }) }}>
        ...
        <div style={{ display: "flex", background: "rgba(255,255,255,0.06)", borderRadius: 10, padding: 3, gap: 3 }}>
          {([["single", "👗 Single", "Phối đồ đơn"] as const, ["batch", "📦 Batch", "Thử nhiều outfit"] as const]).map(([m, label, tip]) => (
            <button key={m} title={tip} onClick={() => setMode(m)} style={btn({
              padding: "6px 14px", fontSize: 12, color: mode === m ? "#fff" : CSS.textSecondary,
      ...
      {/* ── Body ───────────────────────────────────────── */}
      <div style={{ display: "grid", gridTemplateColumns: "290px 1fr 330px", gap: 12, padding: 12, maxWidth: 1600, margin: "0 auto" }}>

        {/* ══ LEFT: Person Panel ═════════════════════════ */}
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={card({ padding: 16 })}>
            <h3 style={{ margin: "0 0 12px", fontSize: 14, fontWeight: 700, color: CSS.accent }}>👤 Nhân vật</h3>
            <ImageUploader image={personImage} onChange={setPersonImage} onZoom={setZoomedImage} height={180} placeholder="Upload / URL ảnh người mẫu" />
            <textarea value={personPrompt} onChange={(e) => setPersonPrompt(e.target.value)}
              placeholder="Mô tả nhân vật: giới tính, vóc dáng, màu da, kiểu tóc..." rows={2}
              style={{ display: "block", width: "100%", marginTop: 10, padding: "8px 12px", borderRadius: CSS.radiusSm, border: CSS.border, background: CSS.bgCard, color: "#fff", fontSize: 12, resize: "none", outline: "none" }} />
          </div>

          {/* Pose */}
          <div style={card({ padding: 16 })}>
            <h3 style={{ margin: "0 0 12px", fontSize: 14, fontWeight: 700, color: CSS.accent }}>🕺 Tư thế</h3>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
              {POSES.map((p) => (
                <button key={p.key} onClick={() => setPose(p.key)} style={btn({
          ...
          <div style={card({ padding: 12 })}>
            <button onClick={() => setShowCleanPrompt(!showCleanPrompt)} style={btn({ background: "transparent", color: CSS.textSecondary, fontSize: 12, padding: "4px 0", display: "flex", alignItems: "center", gap: 6 })}>
              🧹 Prompt làm sạch {showCleanPrompt ? "▲" : "▼"}
            </button>
            {showCleanPrompt && (
              <textarea value={cleanPrompt} onChange={(e) => setCleanPrompt(e.target.value)} rows={3}
                style={{ display: "block", width: "100%", marginTop: 8, padding: "8px 12px", borderRadius: CSS.radiusSm, border: CSS.border, background: CSS.bgCard, color: "#fff", fontSize: 11, resize: "vertical", outline: "none" }} />
            )}
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          >
            {isGenerating ? "⏳ Đang tạo ảnh..." : `✨ ${mode === "single" ? "Thử đồ ngay" : "Thử tất cả outfit"}`}
          </button>
          ...
          {mode === "single" && (
            <>
              <div style={card({ padding: 0, overflow: "hidden", minHeight: 320, display: "flex", alignItems: "center", justifyContent: "center" })}>
                {history.length > 0 ? (
                  <div style={{ position: "relative", width: "100%", cursor: "zoom-in" }} onClick={() => setZoomedImage(history[0].image)}>
                    <img src={history[0].image} style={{ width: "100%", maxHeight: "65vh", objectFit: "contain", display: "block" }} />
                    <div style={{ position: "absolute", top: 10, right: 10 }}>
                      <button onClick={(e) => { e.stopPropagation(); setZoomedImage(history[0].image); }} style={btn({ padding: "6px 12px", background: "rgba(0,0,0,0.6)", color: "#fff", fontSize: 12, border: "1px solid rgba(255,255,255,0.2)", backdropFilter: "blur(8px)" })}>🔍 Zoom</button>
                    </div>
                    <a href={history[0].image} download={`tryon-${history[0].ts}.png`} onClick={(e) => e.stopPropagation()} style={{ position: "absolute", top: 10, left: 10, padding: "6px 12px", background: "rgba(0,0,0,0.6)", color: "#fff", fontSize: 12, border: "1px solid rgba(255,255,255,0.2)", borderRadius: CSS.radiusSm, backdropFilter: "blur(8px)", textDecoration: "none" }}>
                      💾 Tải về
                    </a>
                  </div>
                ) : (
                  <div style={{ textAlign: "center", color: CSS.textMuted, padding: 40 }}>
                    ...
                    <div style={{ fontSize: 12, marginTop: 8 }}>Chọn trang phục và nhấn "Thử đồ ngay"</div>
                  </div>
                )}
              </div>
              ...
              {history.length > 1 && (
                <div>
                  <p style={{ margin: "0 0 8px", fontSize: 12, color: CSS.textMuted, fontWeight: 600 }}>🕐 Lịch sử ({history.length})</p>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(100px, 1fr))", gap: 8 }}>
                    {history.slice(1).map((h) => (
                      <div key={h.id} style={{ position: "relative", borderRadius: CSS.radiusSm, overflow: "hidden", cursor: "pointer", border: CSS.border }} onClick={() => setZoomedImage(h.image)}>
                        <img src={h.image} style={{ width: "100%", height: 80, objectFit: "cover", display: "block" }} />
                        <div style={{ position: "absolute", bottom: 2, right: 4, fontSize: 9, color: "rgba(255,255,255,0.5)" }}>
              ...
              {batches.map((outfit, idx) => (
                <div key={outfit.id} style={card({ padding: 10 })}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: CSS.textPrimary }}>{outfit.name}</span>
                    {batches.length > 1 && (
                      <button onClick={() => setBatches((prev) => prev.filter((o) => o.id !== outfit.id))} style={btn({ padding: "2px 6px", fontSize: 11, background: "rgba(239,68,68,0.15)", color: "#ef4444", border: "1px solid rgba(239,68,68,0.3)" })}>✕</button>
                    )}
                  </div>
                  <div style={{ minHeight: 140, borderRadius: CSS.radiusSm, overflow: "hidden", background: "rgba(0,0,0,0.2)", display: "flex", alignItems: "center", justifyContent: "center", cursor: outfit.result ? "zoom-in" : "default" }}
                    ...
                    {outfit.isGenerating ? (
                      ...
                      <div style={{ color: "#ef4444", fontSize: 11, padding: 12, textAlign: "center" }}>❌ {outfit.error}</div>
                    ) : outfit.result ? (
                      <img src={outfit.result} style={{ width: "100%", height: 140, objectFit: "cover" }} />
                    ) : (
                      <div style={{ color: CSS.textMuted, fontSize: 12, textAlign: "center", padding: 20 }}>
                        <div style={{ fontSize: 28, marginBottom: 4 }}>📦</div>Chưa tạo
                      </div>
                    )}
                  </div>
                  {outfit.result && (
                    <a href={outfit.result} download={`batch-${outfit.name}-${Date.now()}.png`} style={{ display: "block", textAlign: "center", marginTop: 6, padding: "4px 0", borderRadius: CSS.radiusSm, background: "rgba(139,92,246,0.15)", color: CSS.accent, fontSize: 11, textDecoration: "none", border: CSS.borderAccent }}>
                      💾 Tải về
                    </a>
                  )}
                </div>
              ))}
              <button onClick={() => setBatches((prev) => [...prev, makeOutfit(`Outfit ${prev.length + 1}`)])}
                style={card({ padding: 10, border: "2px dashed rgba(139,92,246,0.3)", cursor: "pointer", color: CSS.accent, fontSize: 13, fontWeight: 600, minHeight: 140, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, background: "rgba(139,92,246,0.04)" } as React.CSSProperties)}>
            ...
            <div style={card({ padding: 0, overflow: "hidden" })}>
              ...
              <div style={{ display: "flex", borderBottom: CSS.border }}>
                {ITEM_META.map(({ key, icon, label }) => (
                  <button key={key} onClick={() => setActiveTab(key)} style={btn({
                    flex: 1, padding: "10px 4px", fontSize: 11, borderRadius: 0,
                    background: activeTab === key ? "rgba(139,92,246,0.12)" : "transparent",
                    color: activeTab === key ? CSS.accent : CSS.textMuted,
                    borderBottom: activeTab === key ? `2px solid ${CSS.accent}` : "2px solid transparent",
                  })}>
                    <span style={{ fontSize: 16, display: "block" }}>{icon}</span>{label}
                  </button>
                ))}
              </div>
              ...
              {batches.map((outfit) => (
                <div key={outfit.id} style={card({ padding: 14 })}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                    <input value={outfit.name} onChange={(e) => setBatches((prev) => prev.map((o) => o.id === outfit.id ? { ...o, name: e.target.value } : o))}
                      style={{ flex: 1, background: CSS.bgCard, border: CSS.border, borderRadius: CSS.radiusSm, color: "#fff", padding: "6px 10px", fontSize: 13, fontWeight: 600, outline: "none" }} />
                  </div>
                    ...
                    {ITEM_META.map(({ key, icon, label }) => {
                      ...
                      return (
                        <button key={key} onClick={() => {
                          ...
                          else updateBatchSlot(outfit.id, key, { type: key, image: null, prompt: "" });
                        }} style={btn({
                  ...
                  {Object.entries(outfit.slots).map(([type, slot]) => (
                    <div key={type} style={{ marginBottom: 12 }}>
                      <p style={{ margin: "0 0 6px", fontSize: 12, fontWeight: 700, color: CSS.accent }}>
                        {ITEM_META.find((m) => m.key === type)?.icon} {ITEM_META.find((m) => m.key === type)?.label}
                      </p>
                      <ItemPanel
                        slot={slot!}
                  ...
                  {Object.keys(outfit.slots).length === 0 && (
                    <p style={{ color: CSS.textMuted, fontSize: 12, textAlign: "center", margin: "10px 0" }}>
