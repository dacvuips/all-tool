...
export function ChatList() {
  ...
  const { threads, total, loadMore } = useThreadContext();
  ...
  return (
    ...
    >
      ...
      <i
        ...
        className="absolute z-10 text-gray-400 bg-white border rounded-full right-2 top-1 text-20 hover:text-red-500"
      >
        <RiCloseLine />
      </i>
      {!!thread.customer?.id && !!thread.shop?.id && (
        <i
          ref={createThreadRef}
          className="absolute z-10 text-gray-500 bg-white border rounded-full right-2 top-7 text-20 hover:text-primary-dark"
        >
          <RiArrowDownSFill />
        </i>
      )}
