  ...
  const handleCreateAllVideo = async () => {
    ...
    const result = await runBoundedMediaBatch({
      ...
      execute: async (scene) => {
        ...
        if (!existingImage) {
          if (!scene.visual_prompt) return "skip";
          try {
            addBatchGeneratingSceneId(scene.id);
            reportSceneError?.(scene.id, "image", null);
            const selectedUrls = await getSceneProductImageUrls(scene);
  ...
  const handleRetryAllFailed = async () => {
    ...
    const result = await runBatchRetryWorkerPool({
      ...
      executeTask: async (task) => {
        const scene = task.scene;
        if (task.kind === "image") {
          if (!scene.visual_prompt) return false;
          try {
            addBatchGeneratingSceneId(scene.id);
            reportSceneError?.(scene.id, "image", null);
            const selectedUrls = await getSceneProductImageUrls(scene);
        ...
        if (task.kind === "video") {
          if (!scene.motion_description) return false;
          try {
            let existingImage = await getGeneratedImage(scene.id);
            if (!existingImage && scene.visual_prompt) {
              addBatchGeneratingSceneId(scene.id);
              reportSceneError?.(scene.id, "image", null);
              const selectedUrls = await getSceneProductImageUrls(scene);
              const thumbnailUrl = await getSceneThumbnailUrl(scene.id);
