...
export const SceneBatchRow = React.memo(function SceneBatchRow({
  ...
  return (
    ...
    >
      ...
      {/* ── Media tabs ── */}
      <SceneCardTabs hideImageTab={isPromptToVideo || !!hideImageColumn} hideExtendTab={isPromptToVideo || !nextSceneId}
        renderImageTab={() => (<SceneCardImageTab generatedImage={generatedImage} generatingImage={generatingImage} imageProgress={imageProgress} sceneNumber={scene.sceneNumber} isDisabled={isDisabled} onGenerateImage={handleGenerateImage} onDownloadImage={handleDownloadImage} onSetImage={handleSetImage} onOpenGallery={() => setShowGalleryDialog(true)} />)}
        renderVideoTab={() => (<SceneCardVideoTab generatedVideo={generatedVideo} generatingVideo={generatingVideo} videoProgress={videoProgress} isDisabled={isDisabled} hasImage={!!generatedImage} isPromptToVideo={isPromptToVideo} aspectRatio={scriptData?.aspectRatio} onImageRequired={() => toast.error(t("Cần tạo ảnh trước khi tạo video"))} onGenerateVideo={() => handleGenerateVideo()} onDownloadVideo={handleDownloadVideo} />)}
