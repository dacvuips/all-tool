...
export class SettingRepository extends CrudRepository<Setting> {
  ...
  async getSettingByKey(key: string, fragment?: string) {
    const result = await this.apollo.query({
      query: gql`query { getOneSettingByKey(key: "${key}"){ ${fragment || this.fullFragment} } }`,
    });
    this.handleError(result);
    return result.data["getOneSettingByKey"] as Setting;
  }
  async getSettingNotPrivate() {
    return await this.query({
      query: `getSettingNotPrivate`,
    }).then((res) => res.data["g0"] as Setting[]);
  }
}
