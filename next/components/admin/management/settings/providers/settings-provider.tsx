...
import { Setting, SettingService } from "../../../../../lib/repo";
import {
  SettingGroup,
  SettingGroupService,
} from "../../../../../lib/repo/general/setting-group.repo";
...
export function SettingsProvider(props) {
  ...
  const saveSettings = async (mutableSettings: MutableSetting[]) => {
    ...
    if (!filteredSettings.length) {
      toast.info(t("Chưa có dữ liệu nào thay đổi"));
      return;
    }

    try {
      await SettingService.mutate({
        mutation: filteredSettings.map(
          (setting, i) =>
            `updateSetting(id: "${setting.id}", data: $data${i}) { ${SettingService.fullFragment}}`
        ),
        variablesParams: `(${filteredSettings
          .map((s, i) => `$data${i}: UpdateSettingInput!`)
          .join(", ")})`,
        options: {
          variables: filteredSettings.reduce((acc, setting, i) => {
            acc[`data${i}`] = { value: setting.value };
            return acc;
          }, {}),
        },
      });
      ...
      toast.success(t("Lưu cấu hình thành công"));
    } catch (err) {
      toast.error(t("Lưu cấu hình thất bại. ") + err.message);
    }
  };
  ...
  const deleteSetting = async (setting: Setting) => {
    if (!(await alert.danger(t("Xoá cấu hình"), t("Bạn có chắc chắn muốn xoá cấu hình này?"))))
      return;
  ...
  const deleteSettingGroup = async (settingGroup: SettingGroup) => {
    if (
      !(await alert.danger(
        t("Xoá nhóm cấu hình"),
        t("Bạn có chắc chắn muốn xoá nhóm cấu hình này?")
      ))
    )
      return;
    return await SettingGroupService.delete({ id: settingGroup.id }).then((res) => {
      ...
      if (settingGroup.slug == router.query["slug"]) {
        router.replace(
          "/admin/settings" + (settingGroups.length ? "/" + settingGroups[0].slug : "")
        );
      }
    });
  };
