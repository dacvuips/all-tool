...
type Props = {
  ...
  onCreateItem: (item: FilmSceneImageRecord) => Promise<void>;
...
export default function FilmSceneImagesPanel({
  ...
  const saveEdit = async () => {
    ...
    onItemsChange(items.map((x) => (x.id === updated.id ? updated : x)));
          ...
          {items.length === 0 ? (
            <div className="h-full min-h-2xs flex flex-col items-center justify-center text-center gap-3">
              <p className="text-sm text-gray-500 m-0">
                {t("Chưa có ảnh cảnh. Trích xuất địa điểm từ storyboard hoặc thêm thủ công.")}
              </p>
      ...
      >
        <Dialog.Body>
          <div className="px-5 pt-5 pb-4">
            <h3 className="text-base font-bold text-gray-900 m-0 mb-4">{t("Sửa ảnh cảnh")}</h3>
