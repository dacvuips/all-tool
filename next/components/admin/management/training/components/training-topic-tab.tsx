...
import { PartnerPostDetailDialog } from "./training-dialog";
...
export function TrainingTopicTab({ topicId }) {
  ...
  const GetOnePostSlug = async (page?: Pagination) => {
    setIsLoading(true);
    await PostService.getAllStaffPost({
  ...
  return (
    <>
      ...
      <PartnerPostDetailDialog isOpen={!!postId} onClose={() => setPostId(null)} postId={postId} />
    </>
  );
}
