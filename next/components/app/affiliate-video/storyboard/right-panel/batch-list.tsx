...
export function BatchListPanel({ scenes, characters, storyModeType }: BatchListPanelProps) {
  ...
  const handlePersistScenes = async (updatedScenes: any[]) => {
    try {
      const current = await db.get(CACHE_KEY.lastStoryboardScript);
      await db.set(CACHE_KEY.lastStoryboardScript, {
        ...(current ?? scriptData),
  ...
  const handleSyncScenes = async (updatedScenes: any[]) => {
    try {
      await db.set(CACHE_KEY.lastStoryboardScript, { ...scriptData, scenes: updatedScenes as any });
