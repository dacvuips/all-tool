...
const FilmPage = ({ hideHeader = false }: { hideHeader?: boolean }) => {
  ...
  return (
    <div className="w-full px-4 sm:px-6 lg:px-8 py-8">
      {!hideHeader && (
        <h1 className="text-2xl font-bold text-gray-800 mb-4">{t("Film")}</h1>
      )}
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1.5 m-0">{countLabel}</p>
        </div>
        <Button
          primary
          text={t("Tạo Dự án")}
          icon={<HiPlus />}
          className="!rounded-xl !px-4 !py-2.5 !bg-blue-600 hover:!bg-blue-700 shadow-sm flex-shrink-0"
          onClick={openCreate}
        />
      </div>
