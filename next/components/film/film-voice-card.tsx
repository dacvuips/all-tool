  ...
  return (
    <div
      className={`flex flex-col sm:flex-row sm:items-center gap-2 rounded-xl border px-3 py-2 ${
        take.isDefault ? "border-green-200 bg-green-50/40" : "border-gray-100 bg-gray-50/50"
      }`}
    >
      ...
      <Switch
        dependent
        size="sm"
        value={!!take.isDefault}
        readOnly={!!take.isDefault}
        placeholder={take.isDefault ? t("Mặc định") : t("Đặt mặc định")}
        className="flex-shrink-0 self-end sm:self-auto"
        onChange={async (next) => {
          if (next && !take.isDefault) await onSetDefault?.(take.id);
        }}
      />
    </div>
  );
          ...
          <div className="flex flex-col gap-2">
            {voiceTakes.map((take, i) => (
              <DialogueVoiceTakeRow
                ...
                take={take}
                index={i}
                onSetDefault={(takeId) => onSetDefaultVoiceTake?.(item, takeId)}
              />
            ))}
          </div>
        ) : !creating ? (
