...
function pythonKeyHeaderLine(): string {
  return `${API_MEDIA_KEY_VAR} = "${API_MEDIA_KEY_PLACEHOLDER}"`;
}

export function buildCreateJobSnippet(config: ApiMediaGuideConfig, lang: CodeLang): string {
  ...
  -H "x-api-key: ${API_MEDIA_KEY_PLACEHOLDER}" \\
...
export function buildPollJobSnippet(lang: CodeLang): string {
  const base = getApiBaseUrl();
...
res = requests.post(
    ...
    headers={"x-api-key": ${API_MEDIA_KEY_VAR}, "Content-Type": "application/json"},
    json=${jsonStringify(body).replace(/\n/g, "\n    ")},
    timeout=30,
)
res.raise_for_status()
...
export function buildUpsampleVideoSnippet(lang: CodeLang): string {
  const base = getApiBaseUrl();
