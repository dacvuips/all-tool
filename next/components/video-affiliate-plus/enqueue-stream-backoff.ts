/**
 * Điều phối retry enqueue khi hết slot luồng (429) — tránh spam POST gây 504.
 * Dùng chung mọi luồng video-affiliate-plus trong cùng tab.
 */
import { MediaGenerationJobError } from "../../lib/hooks/useMediaGenerationJob";
...
export function notifyStreamLimitHit(retryAfterMs?: number): void {
  const extra = Number.isFinite(retryAfterMs) && retryAfterMs! > 0 ? retryAfterMs! : MIN_STREAM_RETRY_MS;
