/**
 * Dialog tạo ảnh nhân vật — prompt instruction character sheet.
 */
...
export type FilmCharacterImageGenerateInput = {
  character: FilmCharacterRecord;
  prompt: string;
  /** Ảnh tham chiếu (nhân vật hiện tại + vật phẩm đã chọn, tối đa 10) */
  images?: Array<string | { imageBytes: string; mimeType?: string }>;
  /** Tên/mô tả props gắn vào clothing khi build prompt */
  propNamesInPrompt?: string[];
  /** Id props đã chọn (khớp chính xác khi collect ref) */
  propIds?: string[];
};
...
type Props = {
  ...
  onClose: () => void;
  onGenerate: (input: FilmCharacterImageGenerateInput) => Promise<void>;
};

export default function FilmCharacterImageDialog({
  isOpen,
  character,
  promptTemplate,
  onClose,
  ...
  useEffect(() => {
    ...
    const next =
      ...
      buildFilmCharacterImagePrompt(character, promptTemplate);
    setPrompt(next);
    setSubmitting(false);
  }, [isOpen, character, promptTemplate]);

  if (!character) return null;

  const title =
    character.status === "created" || character.imageUrl || (character.imageUrls?.length || 0) > 0
      ? t("Tạo lại ảnh Nhân vật")
      : t("Tạo ảnh Nhân vật");
  ...
  const handleGenerate = async () => {
    ...
    setSubmitting(true);
    try {
      // Parent chỉ enqueue + mark creating rồi return — job poll nền; đóng dialog ngay.
      await onGenerate({ character, prompt: finalPrompt });
      onClose();
    } catch {
      // parent toast
    } finally {
          ...
          <div className="flex items-start justify-between gap-3 mb-4">
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-500 m-0 mt-1">
                <span className="font-semibold text-gray-700">{character.name}</span>
                {character.role ? (
                  <span className="text-gray-400">
                    {" · "}
                    {filmCharacterRoleLabel(character.role)}
                  </span>
                ) : null}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
              <div className="rounded-xl border border-gray-100 bg-gray-50 px-3 py-2">
                ...
                <div className="text-gray-700 line-clamp-3">
                  {character.description?.trim() || t("Chưa có mô tả")}
                </div>
              </div>
              <div className="rounded-xl border border-gray-100 bg-gray-50 px-3 py-2">
                <div className="text-10 font-semibold text-gray-400 uppercase mb-0.5">
                ...
                <div className="text-gray-700 line-clamp-3">
                  {character.clothingAccessories?.trim() || t("Chưa có trang phục")}
                </div>
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button
                  type="button"
                  onClick={() =>
                    setPrompt(buildFilmCharacterImagePrompt(character, promptTemplate))
                  }
